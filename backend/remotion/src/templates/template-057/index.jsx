  ...
  return (
    <AbsoluteFill style={{ backgroundColor: bg }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, width: '100%', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', padding: '60px', boxSizing: 'border-box' }}>
        ...
        {title && <h1 data-style-role="title" style={mergeStyle({ color: '#fff', fontSize: 48, fontWeight: 700, fontFamily: "'Inter', Arial, sans-serif", textAlign: 'center', margin: 0, marginBottom: 8, opacity: titleO, transform: `translateY(${titleY}px)`, ...positionStyle(overrides.title?.position) }, overrides.title)}>{title}</h1>}
        {subtitle && <p data-style-role="subtitle" style={mergeStyle({ color: '#94a3b8', fontSize: 22, fontFamily: "'Inter', Arial, sans-serif", textAlign: 'center', margin: 0, opacity: subO, ...positionStyle(overrides.subtitle?.position) }, overrides.subtitle)}>{subtitle}</p>}
      </div>

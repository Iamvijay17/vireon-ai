  ...
  return (
    <AbsoluteFill style={{ backgroundColor: bg }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, width: '100%', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', padding: '60px', boxSizing: 'border-box' }}>
        {title && <h1 style={mergeStyle({ color: '#fff', fontSize: 64, fontWeight: 900, fontFamily: "'Orbitron', 'Press Start 2P', monospace", textAlign: 'center', margin: 0, marginBottom: 16, letterSpacing: 4, textShadow: `0 0 30px ${accent}, 0 0 60px ${accent}60`, opacity: titleO, transform: `translateX(${titleX}px)`, ...positionStyle(overrides.title?.position) }, overrides.title)}>{title}</h1>}
        {subtitle && <p style={mergeStyle({ color: accent, fontSize: 24, fontWeight: 600, fontFamily: "'Orbitron', monospace", textAlign: 'center', margin: 0, textTransform: 'uppercase', letterSpacing: 6, opacity: subO, ...positionStyle(overrides.subtitle?.position) }, overrides.subtitle)}>{subtitle}</p>}
      </div>

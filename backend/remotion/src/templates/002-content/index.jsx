...
/**
 * 002-content template ("Card Grid" variant of the "content" scene type)
 *
 * Title + a 2-column grid of cards (heading + text), instead of the
 * 001-content variant's plain vertical bullet list. Good for parallel
 * features/points that don't need to be read in a strict top-to-bottom
 * order. Same elements shape as every other content variant.
 *
 * Data format: same as "001-content" - { title, items: [{heading?, text}] }.
 */
const Content002 = React.memo(({ scene }) => {
  const frame = useCurrentFrame();
  const { fps, width, height } = useVideoConfig();
  const elements = scene?.elements || {};
  const overrides = elements.styleConfig || {};
  const scale = getContentScale(width);
  // The 2-column card grid squeezes hard on a narrow portrait/square
  // canvas - fall back to a single column there instead of shrinking the
  // whole 1920-wide grid to fit.
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: bgColor }}>
      ...
      >
        ...
        <div style={isLandscape ? styles.grid : { ...styles.grid, gridTemplateColumns: '1fr' }}>
          {items.map((item, index) => {
            const cardOpacity = interpolate(frame, [15 + index * 6, 32 + index * 6], [0, 1], { extrapolateRight: 'clamp' });
            const cardY = interpolate(frame, [15 + index * 6, 35 + index * 6], [20, 0], { extrapolateRight: 'clamp' });
            return (
              <div key={index} style={{ ...styles.card, opacity: cardOpacity, transform: `translateY(${cardY}px)` }}>
                <div style={{ ...styles.cardAccent, ...(accentColor ? { background: accentColor } : {}) }} />
...
Content002.displayName = 'Content002';

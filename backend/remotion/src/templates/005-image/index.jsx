...
const Image005 = React.memo(({ scene }) => {
  ...
  const rotation = -4;
  const cardRotate = interpolate(frame, [0, 24], [rotation - 6, rotation], {
    extrapolateRight: "clamp",
  });
  const cardScale = interpolate(frame, [0, 24], [0.85, 1], {
    extrapolateRight: "clamp",
  });
  const cardFade = useFadeIn({ startAt: 0, duration: 18 });
  const stripFade = useFadeIn({ startAt: 20, duration: 15 });

  const labelStyle = mergeStyle(
    { ...styles.label, ...positionStyle(overrides.subtitle?.position) },
    overrides.subtitle,
  );
  const captionStyle = mergeStyle(
    { ...styles.caption, ...positionStyle(overrides.title?.position) },
    overrides.title,
  );
      ...
      <div style={styles.container}>
        ...
        <div
          style={{
            ...styles.card,
            opacity: cardFade,
            transform: `rotate(${cardRotate}deg) scale(${cardScale})`,
          }}
        >
